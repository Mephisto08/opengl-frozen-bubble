...
/*const glm::mat4 Transform::getTransformMatrix(bool isLocal) {
	if (m_matrixDirty) {
		updateTransformMatrix();
		m_matrixDirty = false;
	}

    //glm::mat4 tmp = m_transformMatrix;
    tmp = m_transformMatrix;

    if (isLocal) {
        return m_transformMatrix;
    }

    for (Transform* T = this; T->parent != nullptr; T = T->parent) {
        tmp = T->parent->getTransformMatrix(true) * tmp;
    }
